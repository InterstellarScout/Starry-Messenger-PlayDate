...
| Wacky | Main | A crank-flung inflatable tube figure with Classic and Crazy Family modes. Crazy Family adds full-joint half-size girl and 3/4-size mom Wackys that fling, pull down, and drop like the main figure; the girl has reattaching bows and mom has jointed upper-head hair. Rapidly alternating the crank five times starts party mode with a lowering disco ball, spinning ray lights, and a white/light-grey flashing background. Spinning upward continuously for over five seconds starts Reach for the Stars mode, darkening the background by 30% and adding sparkling stars. Cranking downward continuously for over five seconds latches Worm Mode, flipping the horizon and sliding it near the top so the figures dangle from it; only five seconds of upward crank returns the horizon to normal and re-enables Reach for the Stars. Party and Reach for the Stars turn off after crank bouncing stops for over one second. | The title preview auto-bumps forward every half second. `Left`/`Right` changes Wacky modes. In live play, crank forward flings the figures upward; crank backward pulls them down faster. `A` enables idle auto quarter-turn flails and toggles rigid/pile-up behavior. Optional party audio is reserved at `Source/audio/wacky/partytrack.mp3`. |
| Space Miner | Main | Story Mode asteroid mining and ship combat with shields, missiles, enemy waves, cash, material rarities, configurable asteroid size-layer density/quantity, a configurable world-space ISS-style home base with a proximity-suppressed shield bubble, timed communication popups, cached distant stars/galaxy band, same-layer asteroid breakup collisions, cargo storage, configurable weapon/shield/thruster/cargo upgrades, and a taller white-only bottom dashboard. Space Miner tuning lives in `Source/data/spaceminerconfig.lua`; the Story Mode timeline, communications, and on/off timeline settings live in `Source/data/spaceminerwaves.lua`. Title modes include Story Mode, Continue Mining, New Save, and Ore Mining. Ore Mining only tracks asteroids destroyed and skips Story Mode time events, base systems, communications, and future story changes. | New Story Mode spawns the ship roughly 20 pixels beyond the base shield perimeter in a random cardinal direction. Crank turns the ship. `Up`/`Down` thrusts. Hold `Left` to drill. Press or hold `Right` to launch missiles; Auto Missile Launch fires while held at about a dozen shots per second and spends `$5` per shot. Tiny ore chunks fill the cargo bay instead of paying cash instantly; enter the home base shield to unload cargo through a visible base-to-ship beam that drains ore and adds cash incrementally. While docked, the base repairs shields and hull through a double lightning transfer beam. Cargo capacity grows slowly after each 50 ore collected, and a Cargo Space bar below the minimap flashes when full. Cargo Space upgrades add home-base purchase options from 50 to 300 capacity. When cargo is full, laser mining cannot break the smallest asteroid chunks. `A` opens the ship menu away from base, or the wider home-base menu anywhere inside the base shield. Home base has Scout text plus Laser, Missile, Shield, Thruster, Cargo Space, and Game Settings screens; upgrades must be bought/equipped there. Ship menu and Game Settings include `Trigger Next Event` to jump to the next wave event. Game Settings includes Test Mode, which makes all upgrades freely accessible. Active upgrades change ship, laser, missile, shield, and thruster visuals. With Anti-Grav Drive equipped, hold a D-pad direction and press `A` to blink one screen-width in that direction. Menus pause new wave timing, keep existing threats active, enable light auto-navigation around nearby danger, and save on close; home-base menus also suspend communications until closed. When the Story Mode wave schedule ends, it restarts from the beginning with a white supernova flash. A distant animated white star sits behind foreground objects in the upper-right sky. The base shield destroys asteroid impacts without alerting, drops while the player is inside unless enemies are near, and leaves the base vulnerable to asteroids when down. Base greetings occur once per shield visit; base bumps and player destruction trigger configurable Scout communications with longer display times for long messages. Entity counts are logged periodically for performance diagnostics. After player destruction, enemies destroy nearby asteroids, meander offscreen, and despawn. Story saves player time/location, active stage, stage runtime, cash, score, active/owned upgrades, hull, cargo, and base state at stage checkpoints for Continue Mining; New Save clears the story save. The dashboard shows Shield, ore as `O`, defended enemies as `E`, cash, ship indicator, and 10 hull blocks; Show UI only affects top-left status text. Home menu can toggle compact turn. |
...
| Fractal Tree | Main | An endlessly growing recursive tree that adds branching density and canopy sparkle as crank growth increases, while cranking backward retracts it. | Crank grows or retracts the tree. |
| Brick Builder | Main | A swinging brick stacking game where crank height controls the hoist and poor alignment adds tower lean until the stack tips over. | Crank raises and lowers the swinging brick; lowering into the stack drops it. `A` instantly drops the current brick, or resets after the tower tips. |
| Crank Blocks | Main | A Tetris-inspired falling block toy with crank/A rotation, D-pad movement, line clears, and persistent total row/block counters plus current-session counters. | Crank rotates the active block. `A` also rotates it. `Left`/`Right` slide it, `Down` drops it one row, and `Up` nudges it upward when `GameConfig.crankBlocks.allowUpMove` is enabled. |
| Snake | Main | A forgiving snake game with Standard and Competitive modes, saved player state, crank or D-pad steering, diagonal movement, food, optional NPC snakes, timed player respawns, and screen wrapping. | Crank turns when D-pad is not held. D-pad steers directly, including diagonals. Tap the direction you are already moving to bump forward. The player head shows a direction indicator. `A` opens the Snake menu with NPC Snake 1-4 toggles, Reset Size, and Trim Fat. Each enabled NPC adds one extra board food slot without resetting the map. Competitive uses the same NPC system with NPC Snake 1 locked on as the opponent, starts with 2 food, queues up to 3 before NPC bonuses, despawns defeated NPC snakes, and respawns the player after 5 seconds when the center is available. |
...
| Text Viewer | Main | A simple document reader that opens to a file-selection library. Each piece uses the first two lines of its source as the displayed title and description, supports optional property lines such as `font: sasser-slab`, supports styled markdown headings, hides image/wiki-link lines without leaving extra blank image gaps, includes a Font Sampler document, and includes top-of-reader toggles for cursor and page rolling. Documents without a font property use the saved default font. | Crank or D-pad chooses a document in the library. `A` opens the selected piece, then toggles the line cursor while reading. In Font Sampler, `A` on a font line applies it; `A` on the checkbox controls whether that choice is saved as the default. In the reader, crank moves a highlighted cursor up and down the first letter of each text line and scrolls the page when the cursor reaches mid-page. `Up`/`Down` also move the cursor, `Left` toggles page rolling, `Right` toggles the cursor, and `B` backs from the reader to the library, then to title. |
| Gif Player | Main | A category browser and fullscreen GIF player with frame scrubbing, optional synced audio, and black-outlined white overlay text. | Crank or D-pad chooses categories and GIFs. `A` opens, plays, or cycles playback modes. Crank scrubs frames or changes spin speed. `B` backs out one level. Show UI turns overlay text on or off. |
