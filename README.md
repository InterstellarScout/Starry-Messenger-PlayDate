...
| Space Miner | Main | Story Mode asteroid mining and ship combat with shields, missiles, enemy waves, cash, material rarities, configurable asteroid size-layer density/quantity, a configurable world-space ISS-style home base, timed communication popups, cached distant stars/galaxy band, same-layer asteroid breakup collisions, and a taller white-only bottom dashboard. Space Miner tuning lives in `Source/data/spaceminerconfig.lua`; the Story Mode timeline, communications, and on/off timeline settings live in `Source/data/spaceminerwaves.lua`. Title modes include Story Mode, Continue Mining, New Save, and Ore Mining. Ore Mining only tracks asteroids destroyed and skips Story Mode time events, base systems, communications, and future story changes. | Crank turns the ship. `Up`/`Down` thrusts. Hold `Left` to drill, with the laser capped at 4 enemy penetrations. Press `Right` to launch or detonate a missile. `A` opens the Miner Settings menu with enemy defended count, material marker toggles, and shield upgrade purchases when configured. Story saves player time/location, active stage, stage runtime, cash, score, shield progression, hull, and base state for Continue Mining; New Save clears the story save. The base shows its name when nearby, has a shield-style health bar, flashes `Base under attack` when damaged, and gets an edge arrow with km distance when off-screen. The dashboard shows Shield, ore as `O`, defended enemies as `E`, cash, ship indicator, and 10 hull blocks; Show UI only affects top-left status text. Home menu can toggle compact turn. |
