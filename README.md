...
| Smooth Sailing | Vibe | A Playdate-optimized black-background star tunnel inspired by Warp Speed, with unlimited crank speed, doubled star density, bounded streak drawing, and center/depth-scaled star sizes. | Crank changes speed. Hold D-pad to steer the tunnel. `B` returns to the Vibes folder. |
